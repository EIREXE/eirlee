...
impl FighterStateImpl for GroundAttackState {
    ...
    fn check_interrupt(
        &self,
        state_context: &mut super::FighterStateContext,
    ) -> Option<FighterState> {
    ...
    fn update(&mut self, state_context: &mut super::FighterStateContext) {
        let script = state_context
            .attack_scripts
            .get(&state_context.attack_script_assets.scripts[&self.attack_kind]);
        if let Some(script) = script {
            let animation_frame = state_context.get_current_animation_frame();
            update_active_hitbox_list(
                script,
                &mut state_context.hitboxes.active_hitboxes,
                animation_frame,
            );
        }
    }
    ...
    fn on_enter(&mut self, state_context: &mut super::FighterStateContext) {
        ...
        state_context.hitboxes.attack_script =
            Some(state_context.attack_script_assets.scripts[&self.attack_kind].clone());
