...
pub fn apply_air_drift(velocity: &mut FighterVelocity, attribs: &FighterAttributes, input: &FighterInput) {
    if input.get_last_frame().movement.x.is_zero() {
        ...
        let target_velocity = input.get_last_frame().movement.x.signum() * attribs.max_air_horizontal_velocity;
        let acceleration = input.get_last_frame().movement.x.signum() * attribs.air_acceleration_base;
        let mut acceleration = acceleration + input.get_last_frame().movement.x * attribs.air_acceleration_stick;
